...
#     norms = [int(num) for num, len in zip(pd.Series(match_dict[key]), blon_dict.values())]
#     hmos.loc[key.split('_S')[0]] = [num for num in norms]
# hmos.columns = blon_names
# # hmos.to_csv("~/Downloads/test.csv")
...
# big.to_csv("~/Downloads/testo.csv")
...
# need to drop samples from mothers
bigger = bigger[(bigger["sample"].str.contains("C"))]
